...
<?php 
	...
	/*if( (!empty($_POST['range']['fmonth']) && !empty($_POST['range']['fyear'])) &&
	     (!empty($_POST['range']['tmonth']) && !empty($_POST['range']['tyear'])) )
...
<div align="left">Average rating percentage of the enterprise for selected compliance issues</div>
	...
	<table width="80%" id="myTable" class="tablesorter" cellpadding="0" cellspacing="0" >
		...
		<tr>
			...
			<td><?php echo $this->Html->link('Details', array('controller'=>'surveys','action' => 'facilitysectionans',$company_id,$date)); ?></td>
		</tr>	
