...
<script type="text/javascript">
	...
	function text_toggle(qq){
		var r="gr_"+qq;
		alert('mehedi');
		document.getElementById(r).style.display="block";
	}
	
	function own_text_toggle(qq){
		var r=qq;
		$(r).slideToggle("fast");
	}
...
foreach($gr_sec_lists as $sec => $sec_name){
    ...
    foreach($all_grievances as $key=>$grievance){ 
		if ($grievance['grievances']['section'] == $sec){
            ...
            <?php
            for($day=1;$day<=31;$day++){?>
				<td onclick="text_toggle('<?php echo $gr_id ; ?>_<?php echo $day ; ?>')">
...
<div style=" position:fixed; top:100px; left:400px">
<?php
	foreach($all_grievances as $key=>$grievance){
		$gr_id = $grievance['grievances']['id'];
		for($day=1;$day<=31;$day++){?>
           	<span id="gr_<?php echo $gr_id.'_'.$day ?>" style="display:none">
           	<textarea name="gr[<?php echo $gr_id ; ?>][<?php echo $day ; ?>][comments]" style="width:400px; height:200px"></textarea>
          	</span>
			<?php   
        }
	}
