...
<div class="header_title">
	<?php
		...
		echo $this->Html->link($factory_name, array('controller'=>'users' ,'action' => 'ffactory' , $factory_id)) . " >> ";
		echo "Entry Follow up-".$followUpNumber." Section - ". ($section);
	?>
</div>
...
<?php echo $form->create('Survey', array('action' => 'fprocess','name'=>'test')); ?>


<input type="hidden" value="<?php echo $followUpNumber; ?>" name="followUpNumber"  />
		...
		?>
                <tr>		
                    ...
                    if($question['Survey']['type'] == "3" & empty($question['Answer']))
                        echo '<textarea style="width:90%; height:100%" name="'.$name.'">'.$value.'</textarea>';
