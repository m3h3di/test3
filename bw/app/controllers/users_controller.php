...
class UsersController extends AppController {
	...
	function login() {
		//$this->layout = 'default_sedf';
		$this->layout = 'bwDefault';
	...
	function home() {
		...
		$this->set('factories', $this->User->findAllById($id) );
	}
	function upload() {
	...
	function download() {
		...
		$this->set('questions', $this->Question->findAllBySection(3) );
	...
	function factory($factory_id=NULL) {
		if( !empty($factory_id)){
...
ORDER BY `factory_ans_tables`.`section` ASC") ;*/
			...
			if(!empty($_POST)){
				...
				$this->loadModel("Followup");
				if($this->Followup->query("DELETE FROM `followups` WHERE factory_id = $factory_id ")){
					if($this->Followup->saveAll($data_set))	
						$this->redirect( array('controller' => 'users', 'action' => 'FollowupView',$factory_id,1) ) ;
				}
