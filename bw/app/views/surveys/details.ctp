...
<h2>
	<?php
		...
		$fname= $factory[0]['Factory']['factory_name'];
		
		echo $this->Html->link('Home', array('controller'=>'users' ,'action' => 'home'))." >> ";
		echo $this->Html->link($fname, array('controller'=>'users' ,'action' => 'factory',$fId))." >> ";
		echo "SurveyNo-".$surveyNo;
		$webRoot = $this->webroot;
	?>
</h2>

<p>
<button onclick="location.href='<?= $webRoot?>surveys/editdata/<?= $fId ?>/<?= $surveyNo ?>'">Edit Survey Data</button>
</p>
		...
		<div style="padding:0 0 20px;"><b>Survey Sections</b></div>
			<table width="100%">
				<tr>
					...
					<td style="text-align:center "><b>Section Name</b></td>
					<td style="text-align:center "><b>Total Compliance</b></td>
					<td style="text-align:center "><b>Found</b></td>	
				...
				foreach($surveys as $v){
					$srv = $v['Compliance'];
					$section = $v['Cluster']['name'];
					
					$total+=$srv["total"];
					$found+=$srv["found"];
					$sec = $srv["section"];
					//$sectionName = $this->Html->link($text , array('controller'=>'surveys','action' => 'details',$fId,$surveyNo)); 
					echo '<tr>';
					echo '<td style="text-align:center ">'.$srv["section"].'</td>';
					echo '<td style="text-align:center "><a onclick="showSection('.$sec.')" href="javascript:void(0);">'.$section.'</td>';
					echo '<td style="text-align:center ">'.$srv["total"].'</td>';
					echo '<td style="text-align:center ">'.$srv["found"].'</td>';
				...
				<tr>
					...
					<td style="text-align:center "><b>Aggregation</b></td>
			  ...
			  <div style=" background-color:#EBEBEC;float:left;margin:30px 5px 5px 5px;padding:30px; width:350px;border-radius:8px;-moz-border-radius:8px;-webkit-border-radius:8px;">
					...
					<p style="border-radius:8px;-moz-border-radius:8px;-webkit-border-radius:8px; background:#FFF; padding:20px; font-size:12px">
						...
						echo "Survey No-".$surveyNo."<br/>";
						echo "Survey Date: 14th December <br/><br/>";
						echo 'Rating = (100 * Found)/Total Compliance %<br/>';
						echo 'Rating = (100*'.$found.')/'.$total.' %<br/>';
						if($total)
						echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; = '. round(100*$found/$total,2) .' %<br/>';
						?>
...
if($surveyData){
	...
	<h2 >Survey Data: <span id="sectionTitle" style="font-weight:normal"></span></h2>
	<?php
	...
	foreach($surveyData as $key=>$val){
		...
		?>
			<div class="singleQuestionWrapper" id="section<?=  $cluster['id'] ?>">
							...
							?>
								...
								<div style="padding:10px 0 0 0;text-align:center;height:145px">
									...
									if($question['non-Compliance']){ ?>
										...
										<input <? if($ans['AnswerYesNo'] == 'true') echo 'checked="checked"'; ?> disabled type="radio" name="data[<?= $question['id'] ?>][AnswerYesNo]" onclick="javascript:changeColor(<?= $question['id'] ?>,0)" value="true"> yes
										<input <? if($ans['AnswerYesNo'] == 'false') echo 'checked="checked"'; ?> disabled type="radio" name="data[<?= $question['id'] ?>][AnswerYesNo]" onclick="javascript:changeColor(<?= $question['id'] ?>,1)" value="false"> No
										<br/><br/><br/>
										NC=Y
									<?php
									}
										...
										<input <? if($ans['AnswerYesNo'] == 'true') echo 'checked="checked"'; ?> disabled type="radio" name="data[<?= $question['id'] ?>][AnswerYesNo]" onclick="javascript:changeColor(<?= $question['id'] ?>,1)"  value="true"> yes
										<input <? if($ans['AnswerYesNo'] == 'false') echo 'checked="checked"'; ?> disabled type="radio" name="data[<?= $question['id'] ?>][AnswerYesNo]" onclick="javascript:changeColor(<?= $question['id'] ?>,0)" value="false"> No
										<br/><br/><br/>
										NC=N
									<? } ?>								
...
?>	
	...
	function showSection(id){
		...
		$("div.singleQuestionWrapper").hide();
		$("div#section"+id).slideDown();
		
		$("#sectionTitle").hide();
		var section = myCars[id];
		//alert(section);
		$("#sectionTitle").html(section).fadeIn();
	}
	</script>
	</div>
