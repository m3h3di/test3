...
$(function() {
	...
	$('#SurveyEditdataForm').submit(function(){
...
echo $form->create('Survey', array('action' => 'editdata','name'=>'data'));
?>
...
<div class="singleQuestionWrapper">
	<div class="singleQuestion">
		...
		<div class="singleQuestionLeft">
			...
			<input value="<?=$surveyData[0]['SurveyData']['survey_date']?>" type="text" class="surveydate" value="dd/mm/yy" style="width:100%;margin-top:10px;padding:5px" />
...
foreach($surveyData as $key=>$val){
	...
	$ans = $val['SurveyData'];	//this is survey Ans
	
	$mc= false;
	if($question['id'] == 56 || $question['id'] == 65)	$mc=true; //hard coded for multiple choice questions
		?>
		<div class="singleQuestionWrapper">
			...
			<div class="singleQuestion" id="q<?= $question['id']?>">
				...
				<div class="singleQuestionLeft">
					<fieldset>
						...
						if($question['question_type'] == "Fact-Gathering"){ ?>
							...
							?>
								<div style="float:left;padding-top:10px;width:50%">
									<?php foreach($answers as $k=>$v){
										...
										$tmpMcAns = "[".$mcAns."]";
										?>
										<input <? if(strpos($tmpAns,$tmpMcAns)) echo 'checked'?> onclick="javascript:writeTa(this,<?= $question['id'] ?>,'<?=$mcAns ?>')" type="checkbox" value="true"> <?=$mcAns?> <br/>
										<?php
									}
									?>
								...
								<div style="float:right">
									<textarea disabled id="taDisplay<?=$question['id'] ?>" class="qFindings"><?=$ans['AnswerText']?></textarea>
									<textarea style="display:none" id="ta<?=$question['id'] ?>" name="data[<?= $question['id'] ?>][AnswerText]" class="qFindings"><?=$ans['AnswerText']?></textarea>
								</div>
								<div style="clear:both"></div>
							<?
							}
								...
								<div style="float:left;padding-top:10px;width:50%">
									...
									<input <? if(strpos($tmpAns,"[yes]")) echo 'checked'?> onclick="javascript:writeTa(this,<?= $question['id'] ?>,'yes')" type="checkbox" value="yes"> yes <br/>
							...
							?>
								<textarea name="data[<?= $question['id'] ?>][AnswerText]" class="qFindings"><?=$ans['AnswerText']?></textarea>
							<?
							}
						}
						?>
				...
				<div class="singleQuestionLeft">
					<fieldset>
						...
						<textarea class="qFindings support" name="data[<?= $question['id'] ?>][PersonalNotes]"><?=$ans['PersonalNotes']?></textarea>
						<?php
						if($question['question_type'] != "Fact-Gathering"){ ?>
...
foreach($surveyData as $key=>$val){
	...
	$backColor = "#fff";
	if($ans['CompliantAnswer']== 'false'){
		if($ans['AnswerYesNo'] == 'true') $backColor = "RGB(245,204,202)"; //orrange
		elseif($ans['AnswerYesNo'] == 'false') $backColor = "RGB(209,236,201)"; // green
	}
	if($ans['CompliantAnswer']== 'true'){
		if($ans['AnswerYesNo'] == 'true') $backColor = "RGB(209,236,201)"; // green
		elseif($ans['AnswerYesNo'] == 'false') $backColor = "RGB(245,204,202)"; //orrange
	}
		?>
		<div class="singleQuestionWrapper">
			<input type="hidden" name="data[<?= $question['id'] ?>][factory_id]" value="<?= $factory_id ?>" />
			...
			<div class="singleQuestion" id="q<?= $question['id']?>" style="background:<?=$backColor ?>">
				<div><?= $cluster['name'] ?></div>
				<div class="mainQuestion"><?= $i++ .". ". $question['text'] ?></div>
				<div class="singleQuestionLeft">
					<fieldset>
						...
						<?php
						//print_r($question);
						if($question['question_type'] == "Fact-Gathering"){ 	//this is never going to true  ?>
							<input type="hidden" name="data[<?= $question['id'] ?>][QuestionType]" value="FactGathering" />	<?php
							if(!empty($mc)){
							?>
								<div style="float:left;padding-top:10px;width:50%">
						...
						?>
							...
							<div style="padding:10px 0 0 0;text-align:center;height:145px">
								...
								if($question['non-Compliance']){ ?>
									...
									<input <? if($ans['AnswerYesNo'] == 'true') echo 'checked="checked"'; ?>  type="radio" name="data[<?= $question['id'] ?>][AnswerYesNo]" onclick="javascript:changeColor(<?= $question['id'] ?>,0)" value="true"> yes
									<input <? if($ans['AnswerYesNo'] == 'false') echo 'checked="checked"'; ?>  type="radio" name="data[<?= $question['id'] ?>][AnswerYesNo]" onclick="javascript:changeColor(<?= $question['id'] ?>,1)" value="false"> No
								...
								<?php
									...
									<input <? if($ans['AnswerYesNo'] == 'true') echo 'checked="checked"'; ?>  type="radio" name="data[<?= $question['id'] ?>][AnswerYesNo]" onclick="javascript:changeColor(<?= $question['id'] ?>,1)"  value="true"> yes
									<input <? if($ans['AnswerYesNo'] == 'false') echo 'checked="checked"'; ?>  type="radio" name="data[<?= $question['id'] ?>][AnswerYesNo]" onclick="javascript:changeColor(<?= $question['id'] ?>,0)" value="false"> No
									<input class="inputRadio" type="radio" name="data[<?= $question['id'] ?>][AnswerYesNo]"  value="NA"> NA
									<br/><br/>
									NC=N
								<? } ?>								
								<textarea class="qFindings support" style="" name="data[<?= $question['id'] ?>][AnswerText]"><?=$ans['AnswerText']?></textarea>
							</div>
					...
					<fieldset>
						<div>
							<div style="float:left;padding:10px 0 0 0">
								<input <? if($ans['Observation']=='true') echo 'checked'?>   type="checkbox" name="data[<?= $question['id'] ?>][Observation]" value="true"> Observation
									<input <? if($ans['Documentation']=='true') echo 'checked'?>   type="checkbox" name="data[<?= $question['id'] ?>][Documentation]" value="true"> Document
									<input <? if($ans['Interview']=='true') echo 'checked'?>   type="checkbox" name="data[<?= $question['id'] ?>][Interview]" value="true"> Interview
							</div>
							
							<div style="float:right">
								<input <? if($ans['InterviewWithUnion']=='true') echo 'checked'?>   type="checkbox" name="data[<?= $question['id'] ?>][InterviewWithUnion]" value="true"> Union <br/>
									<input <? if($ans['InterviewWithOther']=='true') echo 'checked'?>   type="checkbox" name="data[<?= $question['id'] ?>][InterviewWithOther]" value="true"> Other
							</div>
							<div style="float:right;margin-right:10px">
								<input <? if($ans['InterviewWithManagement']=='true') echo 'checked'?>   type="checkbox" name="data[<?= $question['id'] ?>][InterviewWithManagement]" value="true"> Management<br/>
									<input <? if($ans['InterviewWithWorker']=='true') echo 'checked'?>   type="checkbox" name="data[<?= $question['id'] ?>][InterviewWithWorker]" value="true"> Worker 
							</div>
						...
						if($question['question_type'] != "Fact-Gathering"){ ?>
						<div style="padding:5px 0 0 0">Level of difficulty: &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<input <? if($ans['lod']=='low') echo 'checked'?> type="radio" name="data[<?= $question['id'] ?>][lod]" value="low"> low
							<input <? if($ans['lod']=='medium') echo 'checked'?> type="radio" name="data[<?= $question['id'] ?>][lod]" value="medium"> medium
							<input <? if($ans['lod']=='high') echo 'checked'?> type="radio" name="data[<?= $question['id'] ?>][lod]" value="high"> high
						</div>
						<?php
						}
						?>
